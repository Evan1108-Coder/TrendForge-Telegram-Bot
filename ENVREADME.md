...
| `MINIMAX_API_KEY` | MiniMax API key for minimax-m2.7, minimax-m2.5-lightning | — |
| `OPENAI_API_KEY` | OpenAI API key for gpt-5.4-pro, gpt-5.4-mini, gpt-4o, gpt-4o-mini | — |
| `ANTHROPIC_API_KEY` | Anthropic API key for claude-opus-4-6, claude-sonnet-4-6, etc. | — |
...
- You need **at least one** LLM API key for AI-powered features (reports, ideas, conversation)
- Without any LLM key, the bot still works for `/trending` and `/hn` (raw data, no AI analysis)
- The default model is `minimax-m2.5-lightning`. Change with `/setmodel` or edit `preferences.json`
